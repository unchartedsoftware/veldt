...
func getTilePromise(tileHash string, tileReq *Request, tileParams map[string]Param) *promise.Promise {
	mutex.Lock()
	...
	runtime.Gosched()
	go func() {
		tileRes := GenerateAndStoreTile(tileHash, tileReq, tileParams)
		p.Resolve(tileRes)
		mutex.Lock()
